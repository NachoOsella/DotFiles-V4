...
export function buildToolRows(
    ...
    const nameWidth = Math.max(
        10,
        Math.min(18, Math.floor(contentWidth * 0.34))
    )
    const countWidth = 6
...
export function buildModelRows(
    ...
    const activeUnattributed =
        unattributed &&
        (modelTokenCount(unattributed) > 0 || unattributed.cost > 0)
            ? unattributed
            : undefined
...
function mergeModels(
    ...
    target.reportedCost =
        (target.reportedCost ?? 0) + (source.reportedCost ?? 0)
    ...
    target.pricedTokens =
        (target.pricedTokens ?? 0) + (source.pricedTokens ?? 0)
