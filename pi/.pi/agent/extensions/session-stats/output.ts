...
export function buildAllStatsOutput(
    ...
    if (totals.estimatedCost > 0)
        costRows.push(
            frame.metric(
                'Estimated value',
                estimatedCostLabel(totals.estimatedCost)
            )
        )
    if (
        ...
        costRows.push(
            frame.metric(
                'Pricing coverage',
                formatPercent(totals.pricingCoverage)
            )
        )
...
export function buildProjectStatsOutput(
    ...
    for (let index = start; index < end; index += 1) {
        ...
        if (!project) continue
        lines.push(
            frame.row(buildProjectRow(project, theme, index === selected))
        )
    }
...
export function buildCurrentSessionOutput(
    ...
    if (stats.toolCalls.length > 0) {
        lines.push(frame.section('Tools'))
        lines.push(
            ...buildToolRows(stats.toolCalls, contentWidth, theme).map(
                frame.row
            )
        )
    }
...
function buildThreadSplitRow(
    ...
    const labelWidth = Math.min(
        14,
        Math.max(8, Math.floor(contentWidth * 0.27))
    )
    const tokenWidth = 8
...
export function calculateAllSessionTotals(
    ...
    for (const session of sessions) {
        ...
        toolCalls += session.toolCalls.reduce(
