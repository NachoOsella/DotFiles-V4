...
function isZenFreeQuotaError(message: unknown): boolean {
    ...
    return /429|freeusagelimiterror|free usage|rate limit exceeded|quota/i.test(
        combined
    )
}

/** Show a clear quota notification without spamming repeated retry attempts. */
function notifyQuotaExceeded(ctx: {
    ui: {
        ...
        void refreshModels(pi)
            ...
            .catch((error) => {
                ...
                if (process.env.PI_STARTUP_PROFILE === '1') {
                    ...
                    console.error(
                        `[pi-startup] pi-zen-free background refresh failed: ${message}`
                    )
                }
            })
    ...
    pi.registerCommand('zen-free-refresh', {
        ...
        handler: async (_args, ctx) => {
            ...
            try {
                ...
                ctx.ui.notify(
                    count > 0
                        ? `Refreshed ${count} Zen free models.`
                        : 'No Zen free models were available.',
