...
// Qwen OAuth endpoint validates User-Agent/X-DashScope-UserAgent format.
// It must look like QwenCode/<version> (...)
const QWEN_USER_AGENT = `QwenCode/0.14.3 (${process.platform}; ${process.arch})`
...
async function pollForToken(
    ...
    while (Date.now() < deadline) {
        ...
        throw new Error(
            'Token request failed: missing access token in response'
        )
    }
...
async function loginQwen(
    ...
    const expiresAt =
        Date.now() + tokenResponse.expires_in * 1000 - 5 * 60 * 1000
...
export default function (pi: ExtensionAPI) {
    pi.registerProvider('qwen-cli', {
        ...
        headers: {
            // Required by portal.qwen.ai for OAuth traffic
        ...
        models: [
            {
                ...
                maxTokens: 65536,
                compat: {
                    // portal.qwen.ai rejects OpenAI's `developer` role with 400 bad request
                    supportsDeveloperRole: false,
                    // qwen-style thinking switch
                    thinkingFormat: 'qwen',
                    // force OpenAI payload to use max_tokens for broad compatibility
            ...
            {
                // Kept for compatibility with older qwen-cli examples
                id: 'vision-model',
                name: 'Qwen3 VL Plus',
                reasoning: true,
                input: ['text', 'image'],
    ...
    // Hardening layer for old sessions/configs:
    // - qwen3.6-plus -> coder-model (Qwen OAuth alias)
    // - developer role -> system (portal.qwen.ai rejects developer role with 400)
    // - max_completion_tokens -> max_tokens (safe fallback for qwen endpoints)
    ...
    pi.on('before_provider_request', (event) => {
        ...
        const model =
            typeof payload.model === 'string' ? (payload.model as string) : ''

        const looksLikeQwenCliModel =
            model === 'coder-model' ||
        ...
        if (selectedProvider !== 'qwen-cli' && !looksLikeQwenCliModel) return

        let changed = false
        const patched: Record<string, unknown> = { ...payload }

        if (patched.model === 'qwen3.6-plus') {
            patched.model = 'coder-model'
            changed = true
        }

        if (Array.isArray(patched.messages)) {
            patched.messages = patched.messages.map((msg) => {
        ...
        if (
            ...
            !Object.prototype.hasOwnProperty.call(patched, 'max_tokens')
        ) {
            patched.max_tokens = patched.max_completion_tokens
