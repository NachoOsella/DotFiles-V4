...
const TodoParams = Type.Object({
    action: StringEnum(
        ['add', 'complete', 'done', 'list', 'show', 'remove', 'clear'] as const,
        {
    ...
    ids: Type.Optional(
        Type.Array(Type.Number(), {
            description: 'Batch todo ids for complete/remove',
        })
    ),
})
...
function reconstructState(
    ...
    options?: { preserveIfNoHistory?: boolean }
): void {
    let foundHistory = false
    let reconstructedTodos: Todo[] = []
    let reconstructedNextId = 1
    ...
    for (const entry of ctx.sessionManager.getBranch()) {
        if (entry.type === 'message') {
            ...
            if (!details) continue
            foundHistory = true
            reconstructedTodos = cloneTodos(details.todos)
            reconstructedNextId = details.nextId
            continue
        }

        if (
            entry.type === 'custom' &&
            entry.customType === 'todo-plan-import'
        ) {
            ...
            reconstructedTodos = cloneTodos(details.todos ?? [])
            reconstructedNextId =
                typeof details.nextId === 'number' && details.nextId > 0
                    ? details.nextId
                    : reconstructedNextId
        }
    }

    if (foundHistory || !options?.preserveIfNoHistory) {
        todos = reconstructedTodos
        nextId = reconstructedNextId
    }
...
function flashTodoWidget(ctx: ExtensionContext): void {
    if (todos.length === 0 || !ctx.hasUI) {
        ctx.ui.setWidget('agent-todo', undefined)
        return
    }

    // Keep the todo tray visible while there are todos (Claude Code-like behavior)
    if (widgetClearTimer) {
...
function updateIndicators(ctx: ExtensionContext): void {
    ...
    // Keep tray in sync and persistent while list is non-empty.
    if (ctx.hasUI) {
        ctx.ui.setWidget('agent-todo', buildTodoLines(ctx.ui.theme))
    }
    ...
    if (allDone()) {
        ctx.ui.setStatus(
            'agent-todo',
            ctx.ui.theme.fg('success', `✓ todos complete (${done}/${total})`)
        )
    } else {
        ctx.ui.setStatus('agent-todo', undefined)
    }
...
const TODO_ENFORCEMENT_SYSTEM_PROMPT =
    'You have pending todos that MUST be completed before you finish. You are NOT allowed to end your response until every todo is marked done. Call the todo tool with action "complete" and the appropriate id/ids now. Do not explain, apologize, or give summaries — just complete the todos.'
...
export default function agentTodo(pi: ExtensionAPI): void {
    ...
    pi.on('session_tree', async (_event, ctx) => {
        latestCtx = ctx
        // Some session tree updates arrive before todo tool-results are fully
        // reflected in history. Preserve in-memory todos in that case.
        reconstructState(ctx, { preserveIfNoHistory: true })
        if (widgetClearTimer) {
            clearTimeout(widgetClearTimer)
            widgetClearTimer = null
        }
    ...
    pi.registerTool({
            ...
            switch (action) {
                case 'add': {
                    ...
                    if (entries.length === 0) {
                        return {
                            content: [
                                {
                                    type: 'text',
                                    text: "Error: 'texts' (or 'text') is required for add",
                                },
                            ],
                            details: {
                                action: 'add',
                    ...
                    const existingKeys = new Set(
                        todos.map((todo) => normalizeTodoKey(todo.text))
                    )
                    const added: Todo[] = []
                    let skipped = 0
                ...
                case 'complete': {
                    ...
                    if (targetIds.length === 0) {
                        return {
                            content: [
                                {
                                    type: 'text',
                                    text: "Error: 'id' or 'ids' is required for complete",
                                },
                            ],
                            details: {
                                action: 'complete',
                    ...
                    const autoCleared =
                        completedSnapshot.length > 0 &&
                        completedSnapshot.every((t) => t.done)
                    if (autoCleared) {
                        resetTodos()
                        updateIndicators(ctx)
                    } else {
                    ...
                    if (completedIds.length === 1)
                        msg = `Done #${completedIds[0]}`
                    else if (completedIds.length > 1)
                        msg = `Done ${completedIds.length} todos: #${completedIds.join(', #')}`
                ...
                case 'remove': {
                    ...
                    if (targetIds.length === 0) {
                        return {
                            content: [
                                {
                                    type: 'text',
                                    text: "Error: 'id' or 'ids' is required for remove",
                                },
                            ],
                            details: {
                                action: 'remove',
                    ...
                    if (removedIds.length === 1)
                        msg = `Removed #${removedIds[0]}`
                    else if (removedIds.length > 1)
                        msg = `Removed ${removedIds.length} todos: #${removedIds.join(', #')}`
                    if (notFoundIds.length > 0)
                        msg +=
                            (msg ? ' | ' : '') +
                            `Not found: #${notFoundIds.join(', #')}`
        ...
        renderCall(args, theme, _context) {
            ...
            const batch = Array.isArray(args.texts)
                ? args.texts.map((t) => `${t}`.trim()).filter(Boolean)
                : []
            if (batch.length > 0) {
                const preview = batch
                    .slice(0, 2)
                ...
                if (batch.length > 2)
                    text += ' ' + theme.fg('muted', `+${batch.length - 2}`)
            } else if (args.text) {
        ...
        renderResult(result, _options, theme, _context) {
            ...
            const doneCount = viewTodos.filter((t) => t.done).length

            if (details.action === 'clear' && total === 0) {
                return new Text(
                    '  ' +
                        theme.fg('borderMuted', '└ ') +
                        theme.fg('dim', 'list cleared'),
                    0,
                    0
                )
            }
            ...
            lines.push(
                '  ' +
                    theme.fg('borderMuted', '└ ') +
                    theme.fg('muted', `${doneCount}/${total}`)
            )
            ...
            for (const t of viewTodos) {
                const icon = t.done
                    ...
                    : theme.fg('success', '☐')
                const txt = t.done
                    ? `\x1b[9m${theme.fg('dim', t.text)}\x1b[29m`
                    : theme.fg('success', t.text)
                lines.push(`    ${icon} ${txt}`)
            }

            if (details.autoCleared && details.action !== 'clear') {
                lines.push(
                    `    ${theme.fg('dim', 'all todos complete · list cleared')}`
                )
            }
        ...
        handler: async (_args, ctx) => {
            ...
            await ctx.ui.custom<void>((tui, theme, _kb, done) => {
                let selectedId: number | null =
                    pendingTodos()[0]?.id ?? todos[0]?.id ?? null
                ...
                const move = (delta: number) => {
                    ...
                    const next = Math.max(
                        0,
                        Math.min(list.length - 1, idx + delta)
                    )
                ...
                const render = (width: number): string[] => {
                    ...
                    if (ordered.length === 0) {
                        ...
                        for (let i = 0; i < ordered.length; i++) {
                            ...
                            const icon = t.done ? '✓' : '☐'
                            const isRecentDone =
                                t.done && lastCompletedId === t.id
                            const iconStyled = t.done
                                ? theme.fg(
                                      isRecentDone ? 'success' : 'text',
                                      icon
                                  )
                    ...
                    add(
                        theme.fg(
                            'dim',
                            '↑↓ / j k move  •  space|enter toggle  •  d remove  •  c clear done  •  esc close'
                        )
                    )
    ...
    pi.on('before_agent_start', async (event) => {
        ...
        if (pending.length > 0) {
            ...
            return {
                systemPrompt:
                    existing +
                    `\n\nTODO: ${pending.length} pending. You MUST complete every pending todo via the todo tool before stopping.\n${pendingList}`,
            }
        }
    ...
    pi.on('agent_end', async (event) => {
        ...
        // Safety cap: stop forcing after MAX_FORCED_TURNS to avoid infinite loops
        if (forcedTurnCount >= MAX_FORCED_TURNS) return
        ...
        // Check if the assistant made any tool calls this turn
        const hasToolCalls = (lastMsg.content || []).some(
            (c: { type: string }) => c.type === 'tool_use'
        )

        // Only force if the assistant ended with pure text (no tool calls = ignored todos)
        if (hasToolCalls) {
            // Agent made progress — reset forced-turn counter
            forcedTurnCount = 0
            return
        }
        ...
        pi.sendMessage(
            {
                ...
                content: `${TODO_ENFORCEMENT_SYSTEM_PROMPT}\n\nPending todos (${pending.length}):\n${pendingList}`,
                display: false,
            },
